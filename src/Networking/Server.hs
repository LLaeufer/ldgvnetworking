...
handleClient :: NMC.ActiveConnections -> MVar.MVar (Map.Map String (NetworkConnection Value Message)) -> MVar.MVar [(String, (Syntax.Type, Syntax.Type))] -> (Socket, SockAddr) -> NC.ConversationOrHandle -> String -> String -> Message -> IO ()
                ...
                Just num -> do
                    ...
                    reply <- case constate of
                        RedirectRequest _ _ host port _ _ _ -> do
                            ...
                            NC.sendResponse hdl (Redirect host port)
                        Connected {} -> case deserialmessages of
                            ...
                            RequestValue userid count -> do
                                ...
                                Data.Maybe.maybe (return False) (\val -> NClient.sendNetworkMessage activeCons ncToPartner (NewValue (ncOwnUserID ncToPartner) count val) 0) mbyval
                                return ()
                            AcknowledgeValue userid count -> do
                                NC.sendResponse hdl Okay -- This okay is needed here to fix a race-condition with disconnects being faster than the okay
                            ...
                            NewPartnerAddress userid port connectionID -> do
                                ...
                                NClient.sendNetworkMessage activeCons ncToPartner (AcknowledgePartnerAddress (ncOwnUserID ncToPartner) connectionID) 0
                                return ()
                            AcknowledgePartnerAddress userid connectionID -> do
                                conConfirmed <- NCon.confirmConnectionID ncToPartner connectionID
                                SSem.signal $ ncHandlingIncomingMessage ncToPartner
                                if conConfirmed then NC.sendResponse hdl Okay else NC.sendResponse hdl Error
                            Disconnect userid -> do
                                NC.sendResponse hdl Okay
                                NCon.disconnectFromPartner ncToPartner
                                SSem.signal $ ncHandlingIncomingMessage ncToPartner
                                -- Config.traceNetIO "Trying to send AcknowledgeDisconnect"
                                -- NClient.sendNetworkMessage activeCons ncToPartner (AcknowledgeDisconnect $ ncOwnUserID ncToPartner) 0
                ...
                case connectionState of
                    ...
                    _ -> do
                        if csConfirmedConnection connectionState then return True else do
                            NClient.sendNetworkMessage activeCons nc (NewPartnerAddress (ncOwnUserID nc) ownport $ csOwnConnectionID connectionState) 0
                            return False
            _ -> return True
...
replaceVChanSerial :: NMC.ActiveConnections -> MVar.MVar (Map.Map String (NetworkConnection Value Message)) -> Value -> IO Value
replaceVChanSerial activeCons mvar input = modifyVChans (handleSerial activeCons mvar) input
    where
        handleSerial :: NMC.ActiveConnections -> MVar.MVar (Map.Map String (NetworkConnection Value Message)) -> Value -> IO Value
